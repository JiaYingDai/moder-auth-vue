...
const handleResend = async () => {
    ...
    try {
        ...
        await axios.post(requestVerificationMailUrl, { email: email.value, callbackUrl: callbackUrl, type: 'forgotpwd' })
        isSent.value = true
        email.value = '';
    } catch (err) {
        console.error(err);
        ...
        if (err.response) {
            ...
            if (errorCode === "User_NotFound") {
                toastMsg = t('PAGE.FORGOT.EMAIL_NOT_FOUND');
            } else {
                // 其他後端錯誤
                ...
                <v-card class="pa-8 text-center" elevation="8" rounded="lg">
                    ...
                    <template v-if="isSent">
                        ...
                        <h2 class="text-h5 font-weight-bold mb-3">
                            {{ t('PAGE.FORGOT.SUCCESS_TITLE') }}
                        </h2>
                        <p class="text-body-1 text-grey-darken-1 mb-6">
                            {{ t('PAGE.FORGOT.SUCCESS_MESSAGE') }}
                        </p>
                    ...
                    <template v-else>
                        <v-avatar size="100" color="#E3F2FD" class="mb-6">
                            <v-icon size="60" color="#658DC6">mdi-email-arrow-right</v-icon>
                        </v-avatar>
                        <h2 class="text-h5 font-weight-bold mb-3">
                            {{ t('PAGE.FORGOT.TITLE') }}
                        </h2>
                        <p class="text-body-1 text-grey-darken-1 mb-6">
                            {{ t('PAGE.FORGOT.DESCRIPTION') }}
                        </p>
